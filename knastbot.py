...
class KnastBot(commands.Bot):
    ...
    async def status_task(self):
        while True:
            await self.change_presence(activity=discord.Game('Destroyer | Dein bot'),
                                      status=discord.Status.online)
            await sleep(20)
            await self.change_presence(activity=discord.Game('Destroyer-Support » version 1.0'), status=discord.Status.online)
            await sleep(20)
            await self.change_presence(activity=discord.Game('Discord » https://discord.gg/pfSHUXDS87'), status=discord.Status.online)
            await sleep(20)
            await self.change_presence(activity=discord.Game('Hilfe » .hilfe'), status=discord.Status.online)
            await sleep(20)
